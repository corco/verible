...
def genyacc(
    ...
    # genrule's `toolchains` attribute is not configurable (no select()
    # allowed), so the platform switch is done one level removed: each
    # alias below picks its `actual` (which IS configurable) and the
    # genrule always lists the same two (now fixed) alias labels.
    native.alias(
