...
Windows) needs no real toolchain there, but the attribute itself can no
longer be a select() (Bazel >= 9): see flex.bzl/bison.bzl, which select
an alias's `actual` between this and the real toolchain instead.
"""
